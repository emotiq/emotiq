...
(defparameter *debug* nil
  "True enables verbose logging and for errors to reach the debugger.")
                                         ...
                                         (handler-case
                                             (progn
                                               (cl-async:start-event-loop #'start-event-loop
                                                                          :catch-app-errors (not *debug*))
                                               (log-event config "cl-async event loop terminated normally"))
                                           (error (err)
                                             (log-event config "cl-async event loop terminated with error" err)
...
(defun inbound-data (socket data)
  ...
  (destructuring-bind (&optional
    ...
    (when *debug* (log-event (config transport) "Received data on socket" socket :transport transport))
    ;;
    ...
    (flet ((transition-to-read-magic ()
      ...
      (setf (cl-async:socket-data socket)
            (list transport state length position buffer)))))
...
(defmethod transmit-message ((transport tcp-async-transport) address port message)
  ...
  (when *debug* (log-event (config transport) "Transmitting message" :address address :port port))
  (with-slots (outbox doorbell) transport
    ...
    (handler-case (cl-async:trigger-notifier doorbell)
      (cl-async:event-error (err)
        ;; Drop message. Transport failure is reported separately via hook.
        ))))
...
(defun process-outbox (transport)
  ...
  (when *debug* (log-event (config transport) "Processing outbox" transport))
...
(defun send-outbound-message (transport address port message)
  ...
  (when *debug* (log-event (config transport) "Transmitting message to " address :port port))
