...
void sha2_finish( sha2_context *ctx, unsigned char output[32] )
{
    ...
    sha2_update( ctx, (unsigned char *) sha2_padding, (int)padn );
...
/*
 * output = SHA_d-256( file contents )
 */
int shad2_file( const char *path, unsigned char output[32], int is224 )
{
    FILE *f;
    size_t n;
    sha2_context ctx;
