...
extern UInt32 rotl(UInt32 x, SInt32 n);
extern UInt32 rotr(UInt32 x, SInt32 n);

inline UInt32 rotl(UInt32 x, SInt32 n)
{
  if(n < 0)
    return rotr(x, -n);
  else
...
inline UInt32 rotr(UInt32 x, SInt32 n)
{
  if(n < 0)
    return rotl(x, -n);
  else
...
#define get_block(x)                            \
    ((UInt32 *)(x))[0] = io_swap(in_blk[3]);     \
    ((UInt32 *)(x))[1] = io_swap(in_blk[2]);     \
    ((UInt32 *)(x))[2] = io_swap(in_blk[1]);     \
    ((UInt32 *)(x))[3] = io_swap(in_blk[0])

#define put_block(x)                            \
    out_blk[3] = io_swap(((UInt32 *)(x))[0]);    \
    out_blk[2] = io_swap(((UInt32 *)(x))[1]);    \
    out_blk[1] = io_swap(((UInt32 *)(x))[2]);    \
    out_blk[0] = io_swap(((UInt32 *)(x))[3])

#define get_key(x,len)                          \
    ((UInt32 *)(x))[4] = ((UInt32 *)(x))[5] =     \
    ((UInt32 *)(x))[6] = ((UInt32 *)(x))[7] = 0;  \
    switch((((len) + 63) / 64)) {               \
    case 2:                                     \
    ((UInt32 *)(x))[0] = io_swap(in_key[3]);     \
    ((UInt32 *)(x))[1] = io_swap(in_key[2]);     \
    ((UInt32 *)(x))[2] = io_swap(in_key[1]);     \
    ((UInt32 *)(x))[3] = io_swap(in_key[0]);     \
    break;                                      \
    case 3:                                     \
    ((UInt32 *)(x))[0] = io_swap(in_key[5]);     \
    ((UInt32 *)(x))[1] = io_swap(in_key[4]);     \
    ((UInt32 *)(x))[2] = io_swap(in_key[3]);     \
    ((UInt32 *)(x))[3] = io_swap(in_key[2]);     \
    ((UInt32 *)(x))[4] = io_swap(in_key[1]);     \
    ((UInt32 *)(x))[5] = io_swap(in_key[0]);     \
    break;                                      \
    case 4:                                     \
    ((UInt32 *)(x))[0] = io_swap(in_key[7]);     \
    ((UInt32 *)(x))[1] = io_swap(in_key[6]);     \
    ((UInt32 *)(x))[2] = io_swap(in_key[5]);     \
    ((UInt32 *)(x))[3] = io_swap(in_key[4]);     \
    ((UInt32 *)(x))[4] = io_swap(in_key[3]);     \
    ((UInt32 *)(x))[5] = io_swap(in_key[2]);     \
    ((UInt32 *)(x))[6] = io_swap(in_key[1]);     \
    ((UInt32 *)(x))[7] = io_swap(in_key[0]);     \
    }

#else

#define get_block(x)                            \
    ((UInt32 *)(x))[0] = io_swap(in_blk[0]);     \
    ((UInt32 *)(x))[1] = io_swap(in_blk[1]);     \
    ((UInt32 *)(x))[2] = io_swap(in_blk[2]);     \
    ((UInt32 *)(x))[3] = io_swap(in_blk[3])

#define put_block(x)                            \
    out_blk[0] = io_swap(((UInt32 *)(x))[0]);    \
    out_blk[1] = io_swap(((UInt32 *)(x))[1]);    \
    out_blk[2] = io_swap(((UInt32 *)(x))[2]);    \
    out_blk[3] = io_swap(((UInt32 *)(x))[3])
...
#define get_key(x,len)                          \
    ...
    switch((((len) + 63) / 64)) {               \
    case 4:                                     \
    ((UInt32 *)(x))[6] = io_swap(in_key[6]);     \
    ((UInt32 *)(x))[7] = io_swap(in_key[7]);     \
    case 3:                                     \
    ((UInt32 *)(x))[4] = io_swap(in_key[4]);     \
    ((UInt32 *)(x))[5] = io_swap(in_key[5]);     \
    case 2:                                     \
    ((UInt32 *)(x))[0] = io_swap(in_key[0]);     \
    ((UInt32 *)(x))[1] = io_swap(in_key[1]);     \
    ((UInt32 *)(x))[2] = io_swap(in_key[2]);     \
    ((UInt32 *)(x))[3] = io_swap(in_key[3]);     \
    }
...
const UInt8  tab_5b[4] = { 0, G_M >> 2, G_M >> 1, (G_M >> 1) ^ (G_M >> 2) };
const UInt8  tab_ef[4] = { 0, (G_M >> 1) ^ (G_M >> 2), G_M >> 1, G_M >> 2 };

#define ffm_01(x)    (x)
#define ffm_5b(x)   ((x) ^ ((x) >> 2) ^ tab_5b[(x) & 3])
#define ffm_ef(x)   ((x) ^ ((x) >> 1) ^ ((x) >> 2) ^ tab_ef[(x) & 3])

const UInt8 ror4[16] = { 0, 8, 1, 9, 2, 10, 3, 11, 4, 12, 5, 13, 6, 14, 7, 15 };
const UInt8 ashx[16] = { 0, 9, 2, 11, 4, 13, 6, 15, 8, 1, 10, 3, 12, 5, 14, 7 };

const UInt8 qt0[2][16] = 
{   { 8, 1, 7, 13, 6, 15, 3, 2, 0, 11, 5, 9, 14, 12, 10, 4 },
    { 2, 8, 11, 13, 15, 7, 6, 14, 3, 1, 9, 4, 0, 10, 12, 5 }
};

const UInt8 qt1[2][16] =
{   { 14, 12, 11, 8, 1, 2, 3, 5, 15, 4, 10, 6, 7, 0, 9, 13 }, 
    { 1, 14, 2, 11, 4, 12, 3, 7, 6, 13, 10, 5, 15, 9, 0, 8 }
};

const UInt8 qt2[2][16] = 
{   { 11, 10, 5, 14, 6, 13, 9, 0, 12, 8, 15, 3, 2, 4, 7, 1 },
    { 4, 12, 7, 5, 1, 6, 9, 10, 0, 14, 13, 8, 2, 11, 3, 15 }
};

const UInt8 qt3[2][16] = 
{   { 13, 7, 15, 4, 1, 2, 6, 14, 9, 11, 3, 0, 8, 5, 12, 10 },
    { 11, 9, 5, 1, 12, 3, 13, 14, 6, 4, 7, 15, 2, 0, 8, 10 }
};

UInt8 qp(const UInt32 n, const UInt8 x)
{
	UInt8  a0, a1, a2, a3, a4, b0, b1, b2, b3, b4;
...
#define f_0(n,x)    ((UInt32)fm_0##n(x))
#define f_1(n,x)    ((UInt32)fm_1##n(x) << 8)
#define f_2(n,x)    ((UInt32)fm_2##n(x) << 16)
#define f_3(n,x)    ((UInt32)fm_3##n(x) << 24)

#define mds(n,x)    f_0(n,q_##n(x)) ^ f_1(n,q_##n(x)) ^ f_2(n,q_##n(x)) ^ f_3(n,q_##n(x))

#endif

UInt32 h_fun(TWOFISH_DATA *pTfd,const UInt32 x, const UInt32 *key)
{
	UInt32  b0, b1, b2, b3;

#ifndef M_TABLE
    UInt32  m5b_b0, m5b_b1, m5b_b2, m5b_b3;
    UInt32  mef_b0, mef_b1, mef_b2, mef_b3;
#endif
...
void gen_mk_tab(TWOFISH_DATA *pTfd,UInt32 *key)
{
	UInt32  i;
    UInt8  by;
...
UInt32 mds_rem(UInt32 p0, UInt32 p1)
{
	UInt32  i, t, u;
...
void Twofish_set_key(TWOFISH_DATA *pTfd,const UInt32 *in_key, const UInt32 key_len)
{
	UInt32  i, a, b, me_key[4], mo_key[4];
...
void Twofish_encrypt(const TWOFISH_DATA *pTfd,const UInt32 *in_blk, UInt32 *out_blk)
{
	UInt32  t0, t1, blk[4];
...
void Twofish_decrypt(const TWOFISH_DATA *pTfd,const UInt32 *in_blk, UInt32 *out_blk)
{
	UInt32  t0, t1, blk[4];
