...
(defun keygen ()
  (multiple-value-bind (private-key public-key)
      (crypto:generate-key-pair +crypto-kind+)
    (let* ((private-key-plist 
             (crypto:destructure-private-key private-key))
           (private-key-octet-vector
             (getf private-key-plist :x))
           (private-key-hex-string
             (octet-vector-to-hex-string private-key-octet-vector))
           (public-key-plist
             (crypto:destructure-public-key public-key))
           (public-key-octet-vector
             (getf public-key-plist :y))
           (public-key-hex-string
             (octet-vector-to-hex-string public-key-octet-vector))
           (public-key-hash-160-string
             (hash-160-string public-key-hex-string)))
      (values
       private-key-hex-string
       public-key-hex-string
       public-key-hash-160-string))))
...
;;; SIGN-MESSAGE: sign MESSAGE, a string (only ASCII text supported at this time
;;; [Mar 24 2018]), using private-key, a hex-string (presumably generated using
;;; KEYGEN function), and return a signature as a hex string of length 128.

(defun sign-message (private-key message &key start end)
  (let* ((private-key-octet-vector
           (hex-string-to-octet-vector private-key))
         (private-key
           (crypto:make-private-key +crypto-kind+ :x private-key-octet-vector))
         (message-octet-vector
           (string-to-octet-vector
            (if (or start end)
                (subseq message (or start 0) end)
                message)))
         (signature-octet-vector
           (crypto:sign-message private-key message-octet-vector)))
    (octet-vector-to-hex-string signature-octet-vector)))



;;; VERIFY-SIGNATURE: verify MESSAGE with SIGNATURE and PUBLIC-KEY. Message and
;;; signature are the same as should have been the arg and result, respectively,
;;; on a prior call to SIGN-MESSAGE function, and public-key was presumably
;;; generated by the same call to the KEYGEN function as was used to generate
;;; the private key for the call to sign-message.  This returns true (non-nil)
;;; if the message is verified, and otherwise false (nil).

(defun verify-signature (public-key message signature &key start end)
  (let* ((public-key-octet-vector
           (hex-string-to-octet-vector public-key))
         (public-key
           (crypto:make-public-key +crypto-kind+ :y public-key-octet-vector))
         (message-octet-vector
           (string-to-octet-vector
            (if (or start end)
                (subseq message (or start 0) end)
                message)))
         (signature-octet-vector
           (hex-string-to-octet-vector signature))
         (signature-as-plist
           (crypto:destructure-signature
            +crypto-kind+ signature-octet-vector))
         (s (getf signature-as-plist :s))
         (r (getf signature-as-plist :r))
         (signature (crypto:make-signature +crypto-kind+ :s s :r r)))
    (crypto:verify-signature public-key message-octet-vector signature)))
