...
(defun check-transactions-hash (blk)
  (vec-repr:int= (merkle-root-hash blk)
...
(defun check-transactions (blk)
  ...
  (and (vec-repr:int= (compute-merkle-root-hash blk) 
                      (merkle-root-hash blk))
       (vec-repr:int= (compute-input-script-merkle-root-hash blk)
                      (input-script-merkle-root-hash blk))
       (vec-repr:int= (compute-witness-merkle-root-hash blk)
