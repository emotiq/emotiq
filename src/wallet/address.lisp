...
(defun encode-address (hash160 version-octet)
  ...
  (let* ((prefix+data
           (concatenate
            'vec-repr:ub8-vector `#(,version-octet) (vec-repr:bev-vec hash160)))
         (checksum-vec
           (checksum-hash-address prefix+data))
         ;; tack that checksum onto the end of the prefix+data
         (prefix+data+checksum
           (concatenate 'vec-repr:ub8-vector prefix+data checksum-vec)))
...
(defmethod address ((pkey pbc:public-key))
  (public-key-to-address pkey))


(defmethod address= ((address-1 string) (address-2 string))
  "ADDRESS= returns true if ADDRESS-1 and ADDRESS-2 are the same.
   Each must be a string, representing an address, that is the public key hash of a public key, 
   which is normally created from a public key via cosi/proofs:public-key-to-address. 
   The strings are compared for exact equality, with no case folding or whitespace elimination. 
   It is an error to pass a non-string to this function."
