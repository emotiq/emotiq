...
;; SDLE-STORE now has a concept of backends.
...
(defclass backend ()
  ((name                     :accessor name
   ...
   (compatible-magic-numbers :accessor compatible-magic-numbers
                             :initarg  :compatible-magic-numbers
                             :type     list)
   
   (old-magic-numbers        :accessor old-magic-numbers
                             :initarg  :old-magic-numbers
                             :type     list)
...
(defun get-store-macro (name)
  ...
  (let ((macro-name (symb 'defstore- name)))
...
(defun get-restore-macro (name)
  ...
  (let ((macro-name (symb 'defrestore- name)))
...
(defun get-class-form (name fields extends)
  `(defclass ,name ,extends
    ,fields
    (:documentation ,(format nil "Autogenerated sdle-store class for backend ~(~A~)."
                             name))))


#+:lispworks
(defun get-dspec-alias-and-parser (name)
  (let ((store-name   (symb 'defstore-   name))
        (restore-name (symb 'defrestore- name)))
...
(defmacro defbackend (name &key
                           (stream-type ''(unsigned-byte 8))
                           (magic-number nil)
                           fields
                           (extends '(backend))
                           (old-magic-numbers nil)
                           (compatible-magic-numbers nil))
