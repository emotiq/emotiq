  ...
  (block body
    (handler-bind ((simple-error
                    #'(lambda (err)
                        (declare (ignore err))
                        (cond
                         ((sdle-store::positive-infinity-p obj)
                          (with-tag ("POSITIVE-INFINITY" stream))
                          (return-from body)) 
                         ((sdle-store::negative-infinity-p obj)
                          (with-tag ("NEGATIVE-INFINITY" stream))
                          (return-from body))
                         ((sdle-store::float-nan-p obj)
...
(defrestore-xml (positive-infinity stream)
  ...
  sdle-store::+positive-infinity+)

(defrestore-xml (negative-infinity stream)
  (declare (ignore stream))
  sdle-store::+negative-infinity+)

(defrestore-xml (float-nan stream)
  (declare (ignore stream))
  sdle-store::+nan-float+)
