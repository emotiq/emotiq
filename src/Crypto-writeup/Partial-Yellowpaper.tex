...
In the following presentation we shall try to adhere to the convention that groups $G_1$ an $G_2$ are additive groups, while $Z_q$ and $G_T$ are both additive and multiplicative. Capitalized symbols denote groups and points from Elliptic curves, while lower cased symbols denote elements of finite fields. Field arithmetic in $Z_q$ is implicitly performed $(mod\,q)$.
...
Mimblewhimble is a simple technique for cloaking all transactions in a publicly verifiable manner. It does not need bilinear pairings, although an algorithm in bilinear pairings can also be developed.\footnote{For Mimblewhimble's single curve computations, we perform in field $Z_q$ and group $G_2$ since that field already has our private keys, and the $G_2$ group has our public keys. Hence the same keying can be used over a single curve algorithm and all of our pairing cryptography.}
